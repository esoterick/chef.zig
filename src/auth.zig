...
const test_str = "Hash this string";
const test_sha1 = "hdBcDGYOo5/Q4k2DojVVP1ANs3U=";
const test_sha256 = "HKxj85/WjYxTHye4B2EPs9UPD8PxhplXZ/tjFucgCj4=";
const test_block = "Stuff and nonsense to encode  ";
const private_key =
...
pub fn sha1(data: [:0]const u8) ![:0]const u8 {
    const sz: usize = data.len;
    const buf: [*:0]u8 = undefined;

    const hash = ssl.SHA1(data, sz, buf);
    const ret: [:0]const u8 = std.mem.span(hash);

    return ret;
}

const RSAError = error{Fuck};

// test "test generate digest signature" {
